...
/*
        ...
        boolean isBlacklisted = blacklistService.isBlacklisted(kundEmail);
        if (!isBlacklisted) {
            bokning.setKund(kund);
            bokningRepo.save(bokning);
            System.out.println("Bokning uppdaterades med kund: " + kundId);
        } else {
 ...
 */
    ...
    private boolean isCustomerBlacklisted(String email) {
        List<BlacklistDto> blacklist = blacklistService.getAllBlacklist();
        return blacklist.stream().anyMatch(blacklistDto -> blacklistDto.getEmail().equals(email) && !blacklistDto.getOk());
    }
    @Override
